 ...
 * }
 * 返り値: { cuts: [{ index: number, phrase: string }] }
 */
export async function POST({ request }) {
	...
	const res = await fetch('https://api.anthropic.com/v1/messages', {
		...
		body: JSON.stringify({
			...
			system:
				...
				'「その部分を編集でカット/やり直したい」と実際に指示・要望している発言だけを選びます。\n' +
				'該当する例(選ぶ): 「ここカットで」「今の無しで」「さっきのところ使わないで」' +
			...
			messages: [
				{
					...
					content:
						...
						'カット/やり直しの指示があるブロックだけを選び、JSON のみで返してください。' +
						'形式: {"cuts":[{"index":0,"phrase":"根拠フレーズ"}]}。無ければ {"cuts":[]}。\n\n' +
						transcript
				}
			]
		})
	...
	let cuts: { index: number; phrase: string }[];
	try {
		const parsed = JSON.parse(match[0]).cuts ?? [];
		cuts = (parsed as unknown[])
			.filter((c): c is { index: number; phrase?: unknown } =>
				typeof (c as { index?: unknown })?.index === 'number')
			.map((c) => ({ index: c.index, phrase: typeof c.phrase === 'string' ? c.phrase : '' }));
	} catch {
